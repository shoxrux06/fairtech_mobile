...
import 'package:fairtech_mobile/src/core/utils/custom_pop.dart';
...
class EnterPricePage extends StatefulWidget {
  const EnterPricePage({super.key, required this.isFromProductDetail, this.dropDownObjectType});

  final bool isFromProductDetail;
  final String? dropDownObjectType;
  @override
...
class _EnterPricePageState extends State<EnterPricePage> {
  ...
  setDropData(){
    dropDownObjectType = widget.dropDownObjectType;
    switch(widget.dropDownObjectType){
      case 'Dehqon bozorlari':
        type = SavdoObyektType.DEXQON.name;
        typeProduct = '';
        isBenzinOrGaz = false;
        code = Code.FOODS.name;
        break;
      case 'Yirik savdo tarmoqlari':
        type = SavdoObyektType.YIRIK.name;
        typeProduct = '';
        isBenzinOrGaz = false;
        code = Code.FOODS.name;
        break;
      case 'Savdo doʼkonlar':
        type = SavdoObyektType.KICHIK.name;
        typeProduct = '';
        isBenzinOrGaz = false;
        code = Code.FOODS.name;
        break;
      case "Yoqilgʼi shahobchalari (Benzin, Dizel)":
        type = SavdoObyektType.BENZIN.name;
        typeProduct = SavdoObyektType.BENZIN.name;
        isBenzinOrGaz = true;
        code = Code.OTHERS.name;
        break;
      case 'Yoqilgʼi shahobchalari (Gaz)':
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: ()async{
        print('ggggg');
        GoRouter.of(context).popUntilPath(context,Routes.productDetail);
        return false;
      },
      child: Scaffold(
          appBar: AppBar(
            title: Text('Narx kiritish'),
            leading: IconButton(
              onPressed: (){
                GoRouter.of(context).popUntilPath(context,Routes.productDetail);
              },
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        ...
                        AppUtils.kGap12,
                        !widget.isFromProductDetail ? Container():SizedBox(
                          height: 54,
                          child: CustomDropDownFormField2(
                            value: dropDownObjectType,
                            hintText: 'Obyekt turi',
                            items: List.from(state.obyektTypeEntity?.list.toSet().toList().map((e) => e.nameLt)??[],),
                            onChanged: (val) {
