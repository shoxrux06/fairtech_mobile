  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ServicesBloc, ServicesState>(
        builder: (context, state) {
          return Scaffold(
            appBar: CustomAppBar(title: state.productInfoResponse != null?'Results': 'TnVed code orqali yuborish'),
            body: ModalProgressHUD(
              inAsyncCall: state.isGettingProductData,
              child: state.productInfoResponse != null? TnVedCodeResponseWidget(productInfoResponse: state.productInfoResponse): Padding(
                padding: const EdgeInsets.all(24.0),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Kode ni kiriting'),
                    AppUtils.kGap4,
                    CustomTextField(
                      ...
                      onFieldSubmitted: (value) {
                        context.read<ServicesBloc>().add(
                          GetProductInfoEvent(
                              onSuccess: () {
                                AppSnackBar.showSuccessSnackBar(context, 'Success');
                              },
                              onError: () {
                                AppSnackBar.showSuccessSnackBar(context, 'Error');
                              },
                              tnVedCode: value
                          ),
                        );
